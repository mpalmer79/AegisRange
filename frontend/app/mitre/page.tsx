  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-gray-100">MITRE ATT&CK Matrix</h1>
      ...
      {loading && (
        <div className="flex items-center justify-center py-20">
          <div className="text-cyan-400 font-mono text-sm animate-pulse">Loading ATT&CK data...</div>
        </div>
      )}
      ...
      {!loading && (
        <>
          ...
          <div className="mb-8">
            <h2 className="text-lg font-semibold text-gray-200 mb-4">Tactic Coverage</h2>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {sortedTactics.map((t) => (
                <div key={t.tactic_id} className="bg-gray-900 border border-gray-800 rounded-lg p-4">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-xs font-mono text-gray-500">{t.tactic_id}</span>
                  </div>
                  <div className="text-sm font-medium text-gray-200 mb-3">{t.tactic_name}</div>
          ...
          {/* ATT&CK Heatmap Grid */}
          <div className="mb-8">
            <h2 className="text-lg font-semibold text-gray-200 mb-4">Coverage Matrix</h2>
            <div className="bg-gray-900 border border-gray-800 rounded-lg overflow-hidden">
              {TACTIC_ORDER.map((tacticId) => {
                ...
                return (
                  <div key={tacticId} className="border-b border-gray-800 last:border-b-0">
                    <div className="px-4 py-3 bg-gray-950/50">
                      <span className="text-xs font-mono text-gray-500 mr-2">{tacticId}</span>
                      <span className="text-sm font-medium text-gray-300">{tacticName}</span>
                    </div>
                    <div className="px-4 py-3 flex flex-wrap gap-2">
                      {entries.length === 0 ? (
                        <span className="text-xs text-gray-600 font-mono">No techniques mapped</span>
                      ) : (
                        entries.map((entry) => (
                          <div
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-800">
                    ...
                    <th className="text-left px-4 py-3 text-xs font-mono text-gray-500 uppercase tracking-wider">ATT&CK Techniques</th>
                    <th className="text-left px-4 py-3 text-xs font-mono text-gray-500 uppercase tracking-wider">Tactics</th>
                    <th className="text-left px-4 py-3 text-xs font-mono text-gray-500 uppercase tracking-wider">Kill Chain Phase</th>
                  </tr>
                </thead>
                <tbody>
                  {mappings.map((m) => (
                    <tr key={m.rule_id} className="border-b border-gray-800/50 hover:bg-gray-800/30">
                      <td className="px-4 py-3 font-mono text-cyan-400 text-xs">{m.rule_id}</td>
                      <td className="px-4 py-3">
                        <div className="flex flex-wrap gap-1">
                          {m.technique_ids.map((t) => (
                            <span key={t} className="px-2 py-0.5 text-xs font-mono bg-purple-500/20 border border-purple-500/30 rounded text-purple-400">
                              {t}
                            </span>
                          ))}
                        </div>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex flex-wrap gap-1">
                          {m.tactic_ids.map((t) => (
                            <span key={t} className="px-2 py-0.5 text-xs font-mono bg-blue-500/20 border border-blue-500/30 rounded text-blue-400">
                              {TACTIC_NAMES[t] || t}
                            </span>
                          ))}
                        </div>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex flex-wrap gap-1">
                          {m.kill_chain_phases.map((p) => (
                            <span key={p} className="px-2 py-0.5 text-xs font-mono bg-amber-500/20 border border-amber-500/30 rounded text-amber-400">
                              {p}
                            </span>
                          ))}
                        </div>
