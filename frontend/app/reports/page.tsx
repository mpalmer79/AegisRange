  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-gray-100">Exercise Reports</h1>
        ...
        <div className="flex flex-col sm:flex-row gap-4 sm:items-end">
          <div className="flex-1">
            ...
            <input
              ...
              className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded text-sm text-gray-200 focus:border-cyan-500 focus:outline-none"
            />
          </div>
          <button
        ...
        <div className="space-y-6">
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-lg p-5">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-bold text-gray-100">{report.title}</h2>
              <span className="text-xs font-mono text-gray-500">{formatTimestamp(report.generated_at)}</span>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
              {[
                ...
                { label: 'Incidents', value: report.summary.total_incidents, color: 'text-red-400' },
                { label: 'Responses', value: report.summary.total_responses, color: 'text-orange-400' },
                { label: 'Scenarios', value: report.summary.scenarios_executed, color: 'text-green-400' },
              ].map((stat) => (
          ...
          {/* Detection Coverage */}
          <div className="bg-gray-900 border border-gray-800 rounded-lg p-5">
            <h3 className="text-lg font-semibold text-gray-200 mb-4">Detection Coverage</h3>
            <div className="flex items-center gap-4 mb-4">
              <div className="text-3xl font-bold font-mono text-cyan-400">
                {report.detection_coverage.rules_triggered}/{report.detection_coverage.rules_total}
              </div>
              <div className="text-sm text-gray-400">detection rules triggered</div>
            </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {report.detection_coverage.rules_list.map((rule: Record<string, unknown>) => (
                  <div key={String(rule.rule_id)} className="flex items-center justify-between px-3 py-2 bg-gray-800/30 rounded">
                    ...
                    <span className={`text-xs font-mono font-bold ${Number(rule.trigger_count) > 0 ? 'text-green-400' : 'text-red-400'}`}>
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-lg p-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <div className="text-center p-3 bg-gray-800/50 rounded-lg">
                <div className="text-2xl font-bold font-mono text-purple-400">{Math.round(report.mitre_coverage.coverage_percentage)}%</div>
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-lg p-5">
            ...
            <div className="space-y-3">
              {report.recommendations.map((rec, i) => (
                <div key={i} className="flex items-start gap-3 p-3 bg-gray-800/30 rounded">
                  <span className="flex-shrink-0 w-6 h-6 bg-cyan-500/20 rounded-full flex items-center justify-center text-xs font-mono text-cyan-400">
                    {i + 1}
                  </span>
