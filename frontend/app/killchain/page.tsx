...
function progressionColor(pct: number): string {
  ...
  if (pct >= 60) return 'text-orange-700 dark:text-orange-400';
  if (pct >= 40) return 'text-amber-700 dark:text-amber-400';
  if (pct >= 20) return 'text-yellow-700 dark:text-yellow-400';
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-gray-100">Cyber Kill Chain</h1>
      ...
      {loading && (
        <div className="flex items-center justify-center py-20">
          <div className="text-cyan-700 dark:text-cyan-400 font-mono text-sm animate-pulse">Loading kill chain data...</div>
        </div>
      )}
      ...
      {!loading && analyses.length === 0 && !error && (
        <div className="bg-white dark:bg-gray-900 border border-slate-200 dark:border-gray-800 rounded-lg p-12 text-center">
          <p className="text-slate-500 dark:text-gray-500 font-mono text-sm">No kill chain data available. Run scenarios to generate incidents.</p>
        </div>
      )}
      ...
      {!loading && sortedAnalyses.length > 0 && (
        <div className="space-y-6">
            ...
            <div className="flex items-center gap-1 overflow-x-auto">
              {Object.entries(STAGE_LABELS).map(([key, label], i) => (
                <div key={key} className="flex items-center gap-1 flex-shrink-0">
                  ...
                  <span className="text-xs font-mono text-slate-600 dark:text-gray-400">{label}</span>
                  {i < Object.keys(STAGE_LABELS).length - 1 && (
                    <svg className="w-3 h-3 text-slate-400 dark:text-gray-600 mx-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          {sortedAnalyses.map((analysis) => (
            <div key={analysis.correlation_id} className="bg-white dark:bg-gray-900 border border-slate-200 dark:border-gray-800 rounded-lg overflow-hidden">
              {/* Header */}
              <div className="px-4 py-3 border-b border-slate-200 dark:border-gray-800 flex flex-wrap items-center justify-between gap-2">
                <div className="flex items-center gap-4">
                  <Link
                    href={`/incidents/${analysis.correlation_id}`}
                    className="font-mono text-cyan-700 dark:text-cyan-400 hover:underline text-sm"
                  >
                    {analysis.correlation_id.slice(0, 16)}...
                  </Link>
                  <span className="text-xs font-mono text-slate-500 dark:text-gray-500">Actor: {analysis.actor_id}</span>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-xs text-slate-500 dark:text-gray-500">progression</span>
                </div>
              </div>
                  ...
                  {analysis.stages.map((stage) => (
                    <div
                      ...
                      className={`flex-1 p-3 rounded border transition-all ${
                        stage.detected
                          ? `${STAGE_COLORS[stage.name]}/20 border-slate-400 dark:border-gray-600`
                          : 'bg-slate-200 dark:bg-gray-800/30 border-slate-200 dark:border-gray-800'
                      }`}
                    >
                      <div className="flex items-center gap-1 mb-1">
                        <div className={`w-2 h-2 rounded-full ${stage.detected ? STAGE_COLORS[stage.name] : 'bg-slate-300 dark:bg-gray-700'}`} />
                        <span className={`text-[10px] font-mono uppercase tracking-wider ${stage.detected ? 'text-slate-800 dark:text-gray-200' : 'text-slate-400 dark:text-gray-600'}`}>
                          {stage.display_name}
                        </span>
                      </div>
                      {stage.detected && stage.detection_rule_ids.length > 0 && (
                        <div className="mt-2 space-y-0.5">
                          {stage.detection_rule_ids.map((r) => (
                            <div key={r} className="text-[9px] font-mono text-slate-600 dark:text-gray-400">{r}</div>
                          ))}
                        </div>
                      )}
                    </div>
              ...
              {/* Footer */}
              <div className="px-4 py-2 border-t border-slate-200 dark:border-gray-800/50 flex flex-wrap items-center justify-between gap-2">
                <span className="text-xs font-mono text-slate-500 dark:text-gray-500">
                  Highest stage: <span className="text-slate-700 dark:text-gray-300">{STAGE_LABELS[analysis.highest_stage] || analysis.highest_stage}</span>
                </span>
                <span className="text-xs font-mono text-slate-500 dark:text-gray-500">
