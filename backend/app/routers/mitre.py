...
@router.get("/mappings", dependencies=[Depends(require_role("viewer"))])
def get_mitre_mappings() -> list[dict]:
    mappings = mitre_service.get_all_mappings()
    return [mitre_mapping_to_dict(m) for m in mappings]


@router.get("/mappings/{rule_id}", dependencies=[Depends(require_role("viewer"))])
...
@router.get("/tactics/coverage", dependencies=[Depends(require_role("viewer"))])
