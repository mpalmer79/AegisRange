...
router = APIRouter(
    tags=["killchain"], responses={401: {"description": "Missing or invalid token"}}
)


@router.get(
    "/killchain",
    response_model=list[KillChainAnalysisResponse],
...
@router.get(
    "/killchain/{correlation_id}",
    response_model=KillChainAnalysisResponse,
    dependencies=[Depends(require_role("viewer"))],
)
